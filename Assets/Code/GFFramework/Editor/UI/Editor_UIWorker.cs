...
using UnityEngine;
using System.Linq;
using Code.Core.GFFramework.SimpleGenCSharpCode;
using System.IO;
using NUnit.Framework.Constraints;

namespace GFFramework.Editor.UI
{
...
//                EditorUtility.DisplayDialog("失败", string.Format("文件夹不存在:{0}", path), "ok");
        ...
        private void OnGUI_EditorItemBindData(UITool_Attribute item, int index)
        {
            ...
            GUI.color = item.GenAttribute_BindData == tpBindNameList[index] ? Color.white : Color.yellow;
        ...
        private void OnSaveButtonClick()
        {
            for (int i = 0; i < itemList.Count; i++)
            {
                ...
                item.GenAttribute_TranformPath = tpIsBindList[i];
                item.GenAttribute_BindData = tpBindNameList[i];
            }
        }
