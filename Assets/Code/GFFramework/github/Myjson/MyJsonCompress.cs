...
public class MyJsonBinary
{
    ...
    /// <param name="puGFict">一个字符串字典(可选)如果字典里有的字符串，保存只记录一个索引，但是需要字典才能读出来</param>
    /// <param name="riseDictByKey">是否把key添加到字典中（默认为false）</param>
    /// <param name="riseDictByString">是否把String值添加到字典中（默认为false）</param>
    public static void Write(System.IO.Stream stream, MyJson.IJsonNode node, IList<string> puGFict = null, bool riseDictByKey = false, bool riseDictByString = false)
    {
        List<string> localdict = new List<string>();
        System.IO.MemoryStream ms = new System.IO.MemoryStream();
        PackJson(ms, node, puGFict, localdict, riseDictByKey, riseDictByString);
    ...
    /// </summary>
    /// <param name="stream">二进制流</param>
    /// <param name="puGFict">一个字符串字典(可选),如果保存的时候用了字典，解压就得用那个字典</param>
    /// <returns></returns>
    public static MyJson.IJsonNode Read(System.IO.Stream stream, IList<string> puGFict = null)
    {
        var list = ReadStrDict(stream);
        return UnPackJson(stream, puGFict, list);
    }
    ...
    static byte MakeStringTag(bool inDict, bool isPuGFict, int keylength)
    {
    ...
    static void WriteStringDataDict(System.IO.Stream stream, bool isPuGFict, int pid)
    {
        int bytelen = 1;
        int c = pid;
        while (c >= 0x100)
        ...
        byte tag = MakeStringTag(true, isPuGFict, bytelen);
        stream.WriteByte(tag);
        byte[] buf = new byte[8];
    ...
    static string ReadString(System.IO.Stream stream, byte tagfirst, IList<string> puGFict, IList<string> localdict)
    {
        bool inDict = (tagfirst & 32) > 0;
        bool isPuGFict = (tagfirst & 16) > 0;
    ...
    static void PackJsonString(System.IO.Stream stream, string str, IList<string> puGFict, IList<string> localdict, bool riseDictByString)
    {
        if (str.Length < 2)
        {//直接写入
    ...
    static void PackJsonArray(System.IO.Stream stream, MyJson.JsonNode_Array array, IList<string> puGFict, IList<string> localdict, bool riseDictByKey, bool riseDictByString)
    {
        WriteArrayCountHead(stream, array.Count);
        for (int i = 0; i < array.Count; i++)
        {
            PackJson(stream, array[i], puGFict, localdict, riseDictByKey, riseDictByString);
        }
    }
    static void PackJsonNode_Object(System.IO.Stream stream, MyJson.JsonNode_Object _object, IList<string> puGFict, IList<string> localdict, bool riseDictByKey, bool riseDictByString)
    {
        ...
        {
            PackJson(stream, item.Value, puGFict, localdict, riseDictByKey, riseDictByString);
        }
    }
    static void PackJson(System.IO.Stream stream, MyJson.IJsonNode node, IList<string> puGFict, IList<string> localdict, bool riseDictByKey, bool riseDictByString)
    {
        if (node is MyJson.JsonNode_ValueString)
        {
            string v = node.AsString();
            if (riseDictByString && v != null && v.Length > 1 && puGFict.Contains(v) == false)
            {
                puGFict.Add(v);
            }
            PackJsonString(stream, v, puGFict, localdict, riseDictByString);
        }
        ...
        {
            PackJsonArray(stream, node as MyJson.JsonNode_Array, puGFict, localdict, riseDictByKey, riseDictByString);
        }
        else if (node is MyJson.JsonNode_Object)
        {
            PackJsonNode_Object(stream, node as MyJson.JsonNode_Object, puGFict, localdict, riseDictByKey, riseDictByString);
        }
    }
    ...
    static MyJson.IJsonNode UnPackJsonString(System.IO.Stream stream, byte tagfirst, IList<string> puGFict, IList<string> localdict)
    {
        MyJson.JsonNode_ValueString str = new MyJson.JsonNode_ValueString();
        str.value = ReadString(stream, tagfirst, puGFict, localdict);
        return str;
    }
    static MyJson.IJsonNode UnPackJsonArray(System.IO.Stream stream, byte tagfirst, IList<string> puGFict, IList<string> localdict)
    {
        MyJson.JsonNode_Array array = new MyJson.JsonNode_Array();
        int count = ReadCountHead(stream, tagfirst);
        for (int i = 0; i < count; i++)
        {
            array.Add(UnPackJson(stream, puGFict, localdict));
        }
        return array;
    }
    static MyJson.IJsonNode UnPackJsonNode_Object(System.IO.Stream stream, byte tagfirst, IList<string> puGFict, IList<string> localdict)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            keys.Add(ReadString(stream, ft, puGFict, localdict));
        }
        for (int i = 0; i < count; i++)
        {
            _object.Add(keys[i], UnPackJson(stream, puGFict, localdict));
        }
        return _object;
    }
    static MyJson.IJsonNode UnPackJson(System.IO.Stream stream, IList<string> puGFict, IList<string> localdict)
    {
        ...
        {
            return UnPackJsonString(stream, b, puGFict, localdict);
        }
        else if (!t1 && !t2)//array
        {
            return UnPackJsonArray(stream, b, puGFict, localdict);
        }
        else//object
        {
            return UnPackJsonNode_Object(stream, b, puGFict, localdict);
        }
    }
