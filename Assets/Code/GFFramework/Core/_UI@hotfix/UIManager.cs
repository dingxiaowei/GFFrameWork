...
namespace GFFramework.UI
{
    ...
    public  class UIManager : ManagerBase<UIManager, UIAttribute>
    {
        ...
        /// UI窗口字典
        /// </summary>
        private Dictionary<int, WindowBase> windowMap = null;
        ...
        {
            //初始化
            windowMap  = new Dictionary<int, WindowBase>();
        ...
        //
        private WindowBase CreateWindow(int uiIndex)
        {
            ...
            var window = Activator.CreateInstance(classData.Type, new object[] { attr.ResourcePath }) as WindowBase;
            //
            return window;
        }
        ...
        public void LoadWindows(params int[] uiIndexs)
        {
            foreach (var i in uiIndexs)
            {
                ...
                if (windowMap.ContainsKey(index))
                {
                    ...
                    if (uvalue.IsLoad)
                    {
                        Debugger.Log("已经加载过并未卸载" +  index, "red");
                    }
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        watch.Stop();
                        Debugger.LogFormat("加载{0},耗时: {1}ms", index, watch.ElapsedMilliseconds);
                        PushCaheData(index);
                    }
        ...
        private void PushCaheData(int uiIndex)
        {
            ...
            if (cacheList != null)
            {
                for (int i = 0; i < cacheList.Count; i++)
                {
                    ...
                    Debugger.Log("push cache data " + uiIndex);
                }
                cacheList.Clear();
                Debugger.LogFormat("推送数据：{0} ,{1}条", uiIndex, cacheList.Count);
            }
        }
        ...
        public WindowBase GetWindow(int uiIndex)
        {
            WindowBase win = null;
