...
public class GFLauncherBridge 
{
    ...
    {   
        Debugger.Log("代码热更:" + isCodeHotfix,"yellow");
        //组件加载
                ...
                if (isCodeHotfix && gameStart == null)
                {
                    ...
                    if (attrs.Length > 0 )
                    {
                        if (attrs[0] is GameStartAtrribute)
                        {
                            ...
                            Debugger.Log("找到启动器" + t.FullName,"red");
                        }
                    }
                }
            }
